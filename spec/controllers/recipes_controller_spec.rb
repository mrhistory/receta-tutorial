...
describe RecipesController do
  ...
  describe 'update' do
    ...
    before do
      xhr :put, :update, format: :json, id: recipe.id, recipe: { name: 'Toast', instructions: 'Add bread to toaster, push lever' }
      recipe.reload
    end
  ...
  describe 'destroy' do
    let(:recipe_id) {
      Recipe.create!(name: 'Baked Potato w/ Cheese', instructions: 'Nuke for 20 minutes; top with cheese').id
    }
    before do
